...
for run in runs:
    ...
    signal_array = np.zeros(4288)   # 6000 for trials 3,4,5  - 4288 for trials 1,2
    center_array = []
    # List input directory files
    files_tot = sorted(os.listdir(path_in))
    files = files_tot
    files = [files_tot[9], files_tot[8]] # Comment to perform batch process over file
    print()
    for file in sorted(files):
        ...
        img_ndfi_filt = img_ndfi*(img_ndfi>0)
        img_ndfi_filt = np.where(img_ndfi_filt>0, 1, 0) # to perform active pixel count 
        img_ndfi_print = imageio.imwrite(os.path.join(path_out,str(file[0:8])+ '_ndfi.png'), img_ndfi_filt)
        
        
        x = np.linspace(0, L, img_ndfi_filt.shape[1])
        
        count = []
        for i in range(0, img_ndfi_filt.shape[1]):
            if i<2826:
                section_cumulate = sum(img_ndfi_filt[40:,i])
            else:
                section_cumulate = 0
        ...
        conv.plot(x, np.flip(count*(count<40)), lw=0.5, label = str(file) + '_filt')
        # conv.legend(fontsize=4)
        plt.show()
